...
def refine_section_labels(sections: list[dict[str, Any]], bar_features: list[dict[str, Any] | Any] | None = None) -> list[dict[str, Any]]:
    ...
    for index, section in enumerate(sections):
        ...
        profile = _profile(section, bar_features)
        section_type, confidence, reasons = _classify(section, profile, previous, nxt, index, len(sections), total)
        item = dict(section)
...
def _classify(
    ...
    next_profile = _profile(nxt) if nxt else None
    prev_profile = _profile(previous) if previous else None
    ...
    if build_score >= 0.34 and nxt is not None and (next_energy_jump > 0.08 or profile["energy_delta"] > 0.08):
    ...
    break_score = (
        (1.0 - profile["drums"]) * 0.22
        + (1.0 - profile["bass"]) * 0.18
    ...
    if post_peak_break or (profile["energy"] < 0.42 and profile["drums"] < 0.45) or break_score >= 0.58 or label == "breakdown_like":
    ...
    repeated_hook = repetition >= 0.62 and profile["vocal"] >= 0.34 and profile["energy"] >= 0.48
    post_build_drop = previous is not None and str(previous.get("sectionType")) == "build" and profile["energy"] >= 0.50
    if drop_score >= 0.72 and (repeated_hook or post_build_drop or label == "drop_like"):
        if repeated_hook:
    ...
    if transition_score >= 0.36 and profile["vocal"] < 0.48:
        return "transition", float(np.clip(0.54 + transition_score * 0.32, 0, 0.88)), ["short-term arrangement change"]

    if profile["vocal"] >= 0.26 or label == "verse_like" or old == "verse":
        return "verse", float(np.clip(0.56 + profile["vocal"] * 0.24 + (1 - repetition) * 0.08, 0, 0.86)), ["lead vocal or narrative section"]
